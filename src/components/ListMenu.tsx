...
interface ListMenuProps {
  ...
  worshipDropdownRef?: React.RefObject<any>;
  currentMenu?: string;
  setCurrentMenu?: React.Dispatch<React.SetStateAction<string>>;
}

const ListMenu: React.FC<ListMenuProps> = ({ isHorizontal, worshipDropdownRef, currentMenu, setCurrentMenu }) => {
  ...
  useEffect(() => {
    const selectorClass = isHorizontal ? 'ul.horizontal-menu' : '.sidebar ul';
    const selector = document.querySelector(`${selectorClass} a[href="${window.location.pathname}"]`);
    if (selector) {
  ...
  return (
    <ul className={isHorizontal ? "horizontal-menu hidden py-1.5 font-semibold px-6 grid grid-cols-2 md:grid-cols-3 gap-4" : "relative font-semibold space-y-0.5 p-4 py-0"}>
      {menuItems.map((item, index) => {
        if (item.subItems) {
                  ...
                  trigger={
                    <button type="button" className="nav-link group w-full">
                      <div className="flex items-center">
                        {item.icon}
                        <span className="ltr:pl-3 rtl:pr-3">{t(item.label)}</span>
                      </div>
                    </button>
                  }
                  position="bottom"
                  ...
                  <div className="text-popover-foreground w-full">
                    <ul>
                      {item.subItems.map((subItem, subIndex) => (
                        <li key={subIndex}>
                          <NavLink to={subItem.to} className="block px-4 py-2 hover:bg-accent">
                            {t(subItem.label)}
                          </NavLink>
                        </li>
                      ))}
            ...
            return (
              <li key={index} className="menu nav-item">
                <button type="button" className={`${currentMenu === 'dashboard' ? 'active' : ''} nav-link group w-full`} onClick={() => toggleMenu('dashboard')}>
                  <div className="flex items-center">
                    {item.icon}
                    <span className="ltr:pl-3 rtl:pr-3 text-sidebar-foreground dark:group-hover:text-sidebar-foreground">{t(item.label)}</span>
                  </div>
                ...
                <AnimateHeight duration={300} height={currentMenu === 'dashboard' ? 'auto' : 0}>
                  <ul className="sub-menu text-muted-foreground">
                    {item.subItems.map((subItem, subIndex) => (
                      <li key={subIndex}>
                        <NavLink to={subItem.to}>{t(subItem.label)}</NavLink>
                      </li>
                    ))}
                  </ul>
                </AnimateHeight>
          ...
          return (
            <li key={index} className="menu nav-item">
              <NavLink to={item.to} className={({ isActive }) => `${isActive ? 'active' : ''} nav-link group w-full`}>
                <div className="flex items-center">
                  {item.icon}
                  <span className={isHorizontal ? "ltr:pl-3 rtl:pr-3" : "ltr:pl-3 rtl:pr-3 text-sidebar-foreground dark:group-hover:text-sidebar-foreground"}>{t(item.label)}</span>
                </div>
              </NavLink>
            </li>
          );
