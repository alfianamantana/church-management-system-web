  ...
  return (
    <div id="dashboard-page" className="space-y-8">
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="stats-section">
        <Card id="total-jemaat-card" className="p-6">
          <div className="text-sm text-muted-foreground" id="total-jemaat-label">{t('total_jemaat')}</div>
          <div className="mt-2 text-2xl font-bold text-primary" id="total-jemaat-value">{totals.totalMembers} {t('jemaat')}</div>
        </Card>
        <Card id="gender-stats-card" className="p-6">
          <div className="flex items-center justify-between" id="gender-stats-content">
            <div className="flex items-center" id="male-stats">
              ...
              <div>
                ...
                <div className="text-2xl font-bold text-primary" id="male-count">{totals.male}</div>
              </div>
            </div>
            <div className="flex items-center" id="female-stats">
              ...
              <div>
                ...
                <div className="text-2xl font-bold text-primary" id="female-count">{totals.female}</div>
              </div>
            </div>
          </div>
        </Card>
        <Card id="total-families-card" className="p-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" id="details-section">
        <Card id="income-card" className="p-6">
          <div className="text-sm text-muted-foreground" id="income-label">{t('income_this_month')}</div>
          <div className="mt-2 text-2xl font-bold text-success" id="income-value">{formatCurrency(finances.incomeThisMonth)}</div>
          <div className="text-xs text-muted-foreground mt-1" id="income-description">{t('income_description')}</div>
        </Card>

        <Card id="expense-card" className="p-6">
          <div className="text-sm text-muted-foreground" id="expense-label">{t('expense_this_month')}</div>
          <div className="mt-2 text-2xl font-bold text-destructive" id="expense-value">{formatCurrency(finances.expenseThisMonth)}</div>
          <div className="text-xs text-muted-foreground mt-1" id="expense-description">{t('expense_description')}</div>
        </Card>

        <Card id="events-card" className="p-6">
        ...
        <Card id="birthdays-card" className="p-6">
          <div className="text-sm text-muted-foreground" id="birthdays-title">{t('birthdays_this_month')}</div>
